...
#'
#' @description
#' Function called by user to calculate criterion. Data should include columns coding
#' for proportion of hits and proportion of false alarms.
#'
#' @param id_var Quoted column name coding for participant ID.
#'
#' @param hit_var Quoted column name coding for porportion of hits.
#'
...
criterion <- function(data,
  ...
  for(i in 1:nrow(data)){
    if(is.infinite(data$criterion[i])){
      data$check[i] <- 1
    } else{
      data$check[i] <- 0
    }
  ...
  if(sum(data$check >= 1)){
    ...
    if("n_hit" %in% colnames(data) == FALSE){
      message("ERROR! Please recalculate proportions, returning raw values, and then recalculate criterion.")
    } else{
